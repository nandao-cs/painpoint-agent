...
5. REFRESH GRAPH: update the "Thesis Map" page under the parent (find it or create it) with a refreshed ```mermaid graph LR of all theses -> segments -> attached companies. Mark AI-Trend theses distinctly (e.g. a ⚡ prefix) so the new-trend cluster is visible.
