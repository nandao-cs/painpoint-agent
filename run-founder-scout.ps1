...
# Minted here, but EXPORTED by Invoke-ClaudeRun after the lane is acquired - see its
# -RunContext note: with BRPX_RUN_ID visible, the lane-abort Telegram alert would be
# routed through act.py's gated, counted path, so a run that never started would
# spend the very budget it is reporting it could not use.
...
#      Harmless only because this file runs under $ErrorActionPreference='Continue';
#      under 'Stop' those same three lines rewrote 124 as 1 thirty-eight times.
#      Leaving a bug latent behind a preference setting is not a fix.
#
# The 30min bound itself (added 2026-08-22) is unchanged. Before it this launcher
...
$code = 1
try {
  $r = Invoke-ClaudeRun -Name 'founder-scout' -Prompt $prompt `
         -DeniedTools $denied -McpConfig $mcpCfg `
         -TimeoutMin 30 -MaxAttempts 3 `
         -LogFile $log -Logger ${function:Log} `
         -TaskLabel 'routine=founder-scout' `
         -RunContext @{
           # BRPX_ROUTINE must match the key in ops/action-policy.json: act.py fails
           # OPEN and warns on an unknown routine, so a typo here silently disarms
           # the gate rather than breaking the run.
           BRPX_RUN_ID  = $brpxRunId
           BRPX_ROUTINE = 'founder-scout'
         }
  $code = $r.ExitCode
}
finally {
